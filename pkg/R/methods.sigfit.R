plot.sigfit <- function(x, table=TRUE, type="fit", norm="weighted", ...) {
    ...
    if (type=="fit") {
        ...
        newdata   = data.frame(value=seq(min(data$value, na.omit=T)*0.1, max(data$value, na.omit=T), length=50))
        ...
        if (table) addtable2plot(0,max(data$MFI)*0.85, data.frame(coefficient=formatC(coefficients(x$fit), digits=9, format="fg")),
            bty="n", cex=0.6, display.rownames=T)
        invisible(newdata)
        }   
    if (type=="resid") {
        xx       = x$data$value
        ...
        print(xx)
        print(yy)
        if (norm == "standardized") { yy = (yy-mean(yy, na.rm=T))/sd(yy, na.rm=T) }
        if (norm == "weighted")    { yy = yy/xx }
        my = max(abs(yy), na.rm=T)*1.05
        print(yy)
...
predict.sigfit <- function(x, newdata=NULL, e.fit=FALSE) {
    # Set stuff
    fit = x$fit
    analyte= x$analyte[1]
    model= x$model[1]
    ...
    if (is.null(newdata)) { data    = x$data }
        ...
        if (any(class(newdata)=="ima") & ncol(newdata)>2) {
            if (x$file!=attr(newdata, "file")) stop("Can't predict data from a different run than the model was built on.")
            fulldata = newdata
...
summary.sigfit <- function(x) {
    s = try(summary(x$fit), silent=T)
    if (class(s)=="try-error") stop(ifelse(x$model["equation"]=="L","Logistic","Hill")," type ", 
    x$model["type"], " fit for ", x$model["analyte"]," - invalid.") 
    else {        
        d            = x$qcs
        if (nrow(d)>0) { d$weights    = d$use = NA }
        d            = rbind(x$data, d)
        d$predicted  = predict(x, newdata=d)
        ...
        invisible(list(fit=s, res=d, stats=x$stats))
        }
}
